...
public class FirstPersonController : MonoBehaviour
{
    ...
    // Animator and related to animations
    ...
    private void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            if (targetVelocity.x != 0 || targetVelocity.z != 0 && isGrounded)
            {
                ...
                {
                    ChangeAnimationState("Walking backwards");
                }
                else if(targetVelocity.x < 0)
                {
                    ChangeAnimationState("Left Strafe Walking");
                }
                else if(targetVelocity.x > 0)
                {
                    ChangeAnimationState("Right Strafe Walking");
                }
            }
            else
            {
    ...
    {
        Vector3 origin = new Vector3(transform.position.x, transform.position.y - ((transform.localScale.y-1.4f) * .5f), transform.position.z);
